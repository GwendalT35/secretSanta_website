window.onload = () => {
    ...
    alert("De nouveau devant une table, ce n'est pas sans vous rappeler une énigme auparavant...\nVous vous élancez en sachant quoi faire")

    let cardsContainer = document.querySelector('.cards');
    let imgArr = ['couronne.png', 'barillet.jpg', 'engrenage.png', 'echappement.gif', 'balancier.jpg', 'engrenage.png', "horloge.jpg"];
    ...
    function handleDrop(e) {
        ...
        if (dragSrcEl !== this && droppedBox.classList.contains('box')) {
            ...
            console.log(imgAns, ": ", JSON.stringify(['couronne', 'barillet', 'engrenage', 'echappement', 'balancier', 'engrenage', 'horloge'] ), " = ", JSON.stringify(imgAns) == JSON.stringify(['couronne', 'barillet', 'engrenage', 'echappement', 'balancier', 'engrenage', 'horloge'] ))
            if (JSON.stringify(imgAns) == JSON.stringify(['couronne', 'barillet', 'engrenage', 'echappement', 'balancier', 'engrenage', 'horloge'] )){
                if (nbEnigmSolved == 6) localStorage.setItem("nbEnigmSolved", nbEnigmSolved+1)
                alert("Décidément vous connaissez comment fonctionne une montre mécanique...")
