window.onload = () => {
    ...
    alert("Encore une fois, une table avec des instruments de mesures : cette \nfois-ci, je me demande comment les ranger.")
    let cardsContainer = document.querySelector('.cards');

    let imgArr = ['Huygens_clock.png', 'harrison_chronometer.jpg', 'montre_a_quartz.jpg', 'atomic_commercial.jpg', 'fontaine_cesium.jpg', 'horloge_optique.jpg'];
    ...
    function handleDrop(e) {
        ...
        if (dragSrcEl !== this && droppedBox.classList.contains('box')) {
            ...
            console.log(imgAns, ": ", JSON.stringify(imgAns) == JSON.stringify(['Huygens_clock', 'harrison_chronometer', 'montre_a_quartz', 'atomic_commercial', 'fontaine_cesium', 'horloge_optique']))
            if (JSON.stringify(imgAns) == JSON.stringify(['Huygens_clock', 'harrison_chronometer', 'montre_a_quartz', 'atomic_commercial', 'fontaine_cesium', 'horloge_optique'])){
                if (nbEnigmSolved == 10) localStorage.setItem("nbEnigmSolved", nbEnigmSolved+1)
                alert("Vous êtes à présent maître de la précision !")
